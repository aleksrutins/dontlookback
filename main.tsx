...
const framesSpan  = document.querySelector('#timer'),
        ...
        introDiv: HTMLDivElement = document.querySelector('#intro')!,
        happyBday   = document.querySelector('#happyBirthdayYoshi')!;
    
if(localStorage.getItem('stage') != 'ending') {
    await typeText(happyBday, "Happy Birthday Yoshi");
    happyBday.textContent = "";
    ...
    game.mainLoop(() => {
        ...
        if(game.get(Player, 'player') == null) {
            ...
            localStorage.setItem('stage', 'ending');
            location.reload();
        }
    ...
    await pause(1000);
    introDiv?.parentNode?.removeChild(introDiv);
    const frames = parseInt(localStorage.getItem('frames') ?? '0');
    ReactDOM.render(<Ending score={frames}/>, document.body);
}